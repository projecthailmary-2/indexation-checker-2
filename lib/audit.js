...
export async function fetchSerpCount(domain) {
  ...
  try {
    ...
    if (json.error) {
      ...
      if (low.includes('credits')) return 'No Credits';
...
export async function auditDomain(domain) {
  ...
  // Step 1: WordPress post count.
  const { value: wp, reason } = await fetchWPPostCount(d);
  if (!Number.isFinite(wp)) {
    return { row: { domain: d, wpCount: '-', serpCount: '-', rate: '-', failed: true, failReason: reason || 'Unreachable' }, posts: [], creditIssue: null };
  }

  // Step 2: SerpApi site: count.
  const serpCount = await fetchSerpCount(d);
  if (serpCount === 'No Credits' || serpCount === 'Invalid Key') {
    return { row: { domain: d }, posts: [], creditIssue: serpCount };
  }
  const serp = parseInt(serpCount);
  const rate = (wp > 0 && Number.isFinite(serp)) ? Math.min(serp / wp, 1.0) : 0;
  ...
  if (!Number.isFinite(parseInt(count))) {
    return { row: { domain: d, wpCount: '-', serpCount: '-', rate: '-', failed: true, failReason: pReason || 'Unreachable' }, posts: [], creditIssue: null };
  }
  const posts = [];
  ...
  for (const p of posts) {
    ...
    if (status === 'No Credits' || status === 'Invalid Key') {
      return { row: { domain: d }, posts: [], creditIssue: status };
    }
  ...
  const row = { domain: d, wpCount: wp, serpCount: serp, rate, failed: false, ...stats };
  return { row, posts, creditIssue: null };
}
