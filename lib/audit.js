...
export async function fetchPostLinks(domain) {
  ...
  // Pull each post's rendered body HTML straight from the WP API response (the
  // `content.rendered` field) instead of a second fetch to the live page — see
  // analyzePost() for why.
  const links = [];
  const collect = posts => { for (const p of posts) links.push({ url: p.link, pubDate: p.date ? p.date.split('T')[0] : '', html: p.content?.rendered ?? '' }); };
  collect(first.posts);
...
// Takes the post's rendered body HTML (WordPress REST API's `content.rendered`
// field, already fetched by fetchPostLinks) rather than fetching the live post
// URL ourselves. Two reasons: it's the CLEAN way to get this — the WP REST API
// is a documented, structured data source, vs. fetching a page built for a
// human browser and regex-mining it, which is the "web scraping" pattern
// PaaS acceptable-use policies (Vercel included) flag. And it's naturally
// scoped to just the post body already — no header/sidebar/footer chrome to
// accidentally sweep a YouTube link or external link out of.
// Returns { externalCount, taskType }.
// ---------------------------------------------------------------------------
export function analyzePost(content, sourceDomain) {
  try {
    // A protected (password-locked) post has no body to analyze — the site
    // owner didn't expose it. Report it the same way as an unreadable post
    // rather than guessing.
    if (!content) return { externalCount: 0, taskType: 'Error' };
    ...
    // A real Video Bridge is built around a video and won't have 10+ body links,
    // so the link count is the stronger signal — check Sequoia FIRST so a
    // link-heavy post with an incidental YouTube link isn't stolen into VB.
    let taskType = 'Others';
    if (externalCount >= 10) taskType = 'Sequoia';
    else if (hasYouTube) taskType = 'Video Bridge';
...
export async function auditDomain(domain) {
  ...
  for (const { url, pubDate, html } of links) {
    const { externalCount, taskType } = analyzePost(html, d);
