...
export function aggregateHistory(rows, { period = 'week', libraryTotal = 0, now = new Date() } = {}) {
  ...
  // Bucket by period; within a bucket keep the LATEST snapshot per site
  // (so a site audited twice in one period isn't double-counted).
