...
const SUMMARY_KEY = 'runner:lastSummary';
...
// Last weekly summary posted to Slack: { week: 'YYYY-MM-DD', coverage: 0.99 }.
// Lets the runner post once per week and re-post only when coverage improves.
export async function getLastSummary() {
  const r = client();
  if (!r) return null;
  return (await r.get(SUMMARY_KEY)) || null;
}
export async function setLastSummary(obj) {
  const r = client();
  if (!r) return;
  await r.set(SUMMARY_KEY, obj);
}
