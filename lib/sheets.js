...
// A Google Sheets checkbox reads back as boolean true (or "TRUE" in FORMULA mode);
// also accept a hand-typed x / yes / 1 so the "Recheck?" flag is forgiving.
function isTruthy(v) {
  if (v === true) return true;
  const s = String(v ?? '').trim().toLowerCase();
  return s === 'true' || s === 'x' || s === 'yes' || s === 'y' || s === '✓' || s === '1';
}
...
function mapColumns(rows) {
  ...
  const cols = {
    ...
    recheck:       find(h => h.includes('recheck')),
  };
...
function parseDateMs(s) {
  ...
  // The runner stamps dates as UTC calendar dates (M/D/YYYY). Parse them back as
  // UTC — NOT via Date.parse, which reads "8/9/2026" in the LOCAL zone and can land
  // it a few hours off the UTC week boundary, wrongly flagging a Sunday-audited
  // site as "not done this week". Fall back to Date.parse for other formats.
...
export async function getTrackingSites({ limit = 0, failedOnly = false, freshDays = 0, weekGate = false } = {}) {
  ...
  for (let i = headerRow + 1; i < rows.length; i++) {
    ...
    // "Recheck?" checkbox: a human force-audit for THIS site — overrides the
    // freshness gate and the status skip, and jumps to the front of the queue.
    const recheck = cols.recheck >= 0 ? isTruthy((rows[i] || [])[cols.recheck]) : false;
    sites.push({ domain: d, normDomain: normDomain(d), lastChecked: rawDate, ts: Number.isNaN(ts) ? -Infinity : ts, flagged, status: flagged ? indexedStr : '', auditable, skipAudit, recheck });
  }
  ...
  const recheckCount = pool.filter(s => s.recheck).length;
  // Status gate: exclude "Down - Not in Quarantine" sites — unless explicitly rechecked.
  const skippedByStatus = pool.filter(s => s.skipAudit && !s.recheck).length;
  pool = pool.filter(s => !s.skipAudit || s.recheck);

  // Eligibility gate. DEFAULT (weekGate): a site is due if it hasn't been audited
  // THIS week — i.e. its last check predates the most recent Sunday (UTC). This is
  // robust to slow/late passes: a delayed run can never leave a site looking
  // "fresh" and skip it (the flaw in a rolling N-day window). A positive freshDays
  // OVERRIDES the week gate with a rolling window (for one-off forced re-audits).
  // Never-checked sites, and any rechecked site, always stay eligible.
  if (freshDays > 0) {
    const cutoff = Date.now() - freshDays * 86400000;
    pool = pool.filter(s => s.recheck || s.ts < cutoff);
  } else if (weekGate) {
    const n = new Date();
    const startOfWeek = Date.UTC(n.getUTCFullYear(), n.getUTCMonth(), n.getUTCDate() - n.getUTCDay());
    pool = pool.filter(s => s.recheck || s.ts < startOfWeek);
  }

  // Rechecked sites jump the queue so a manual force-audit runs in the next batch.
  pool.sort((a, b) => (Number(b.recheck) - Number(a.recheck)) || (a.ts - b.ts));
  ...
  return {
    ...
    total, auditableTotal, auditableDomains, skippedByStatus, recheckCount, neverChecked, flaggedCount, eligible, freshDays, failedOnly,
...
export async function writeTrackingResults(results) {
  ...
  for (const r of results) {
    ...
    // Auto-untick the "Recheck?" box now that the site has been (re)audited, so the
    // human doesn't have to clear it and it won't force-audit again next run.
    if (cols.recheck >= 0) {
      const cur = existing(rowIdx, cols.recheck);
      if (isTruthy(cur) && !isFormula(cur)) {
        data.push({ range: `${qtab(tab)}!${colLetter(cols.recheck)}${rowIdx + 1}`, values: [[false]] });
        wroteAny = true;
      }
    }
...
// ===== Per-type audit logs (append-only) =====
// Every audited Sequoia / Video Bridge post — indexed AND unindexed — so the
// classification (link count) and the indexation verdict can both be spot-
// checked by hand. Unlike the Salvage tab (unindexed only), these are the full
// record. Same columns for both; External Links helps verify classification
// (Sequoia should be 10+, a real Video Bridge should be under 10).
const POSTLOG_HEADERS = [
...
const VBLOG_TAB = () => process.env.GOOGLE_VBLOG_TAB || 'Video Bridge Audit Log';

// posts: [{ domain, link, pubDate, externalCount, indexStatus }]
async function appendPostLog(tab, posts) {
  if (!Array.isArray(posts) || posts.length === 0) return { tab, appended: 0 };
  await ensureTab(tab, POSTLOG_HEADERS);
  const t = new Date();
  const dateStr = `${t.getMonth() + 1}/${t.getDate()}/${t.getFullYear()}`;
  const rows = posts.map(p => [
    p.domain ?? '', p.link ?? '', p.pubDate ?? '',
...
export const appendSequoiaLog = posts => appendPostLog(SEQLOG_TAB(), posts);
export const appendVideoBridgeLog = posts => appendPostLog(VBLOG_TAB(), posts);

// Trim an append-only audit log to the last `keepDays` days. Rows are written in
// audit order (oldest at the top), so everything older than the cutoff is one
// contiguous block at the top — we delete it in a single request. We stop at the
// first row that's recent OR has an unparseable date, so bad data can never cause
// a runaway delete.
export async function pruneLogTab(tab, keepDays = 14) {
  const meta = await api('?fields=sheets(properties(sheetId,title))');
  const sheet = (meta.sheets || []).find(s => s.properties.title === tab);
  if (!sheet) return { tab, pruned: 0 };                 // tab not created yet
  const vals = (await api(`/values/${encodeURIComponent(qtab(tab))}!A1:F`)).values || [];
  if (vals.length <= 1) return { tab, pruned: 0 };
  const di = (vals[0] || []).indexOf('Date Audited');
  if (di < 0) return { tab, pruned: 0 };
  const today0 = new Date(); today0.setUTCHours(0, 0, 0, 0);
  const cutoff = today0.getTime() - keepDays * 86400000; // midnight-aligned day count
  let cut = 0; // count of definitely-old rows at the top to delete
  for (let i = 1; i < vals.length; i++) {
    const t = Date.parse((vals[i] || [])[di]);
    if (Number.isNaN(t) || t >= cutoff) break;           // stop at first recent/unknown row
    cut = i;
  }
  if (cut <= 0) return { tab, pruned: 0 };
  await api(':batchUpdate', 'POST', {
    requests: [{ deleteDimension: { range: { sheetId: sheet.properties.sheetId, dimension: 'ROWS', startIndex: 1, endIndex: cut + 1 } } }],
  });
  return { tab, pruned: cut };
}

// Prune both audit logs; never throws (a prune failure must not fail a run).
export async function pruneAuditLogs(keepDays = 14) {
  const out = [];
  for (const tab of [SEQLOG_TAB(), VBLOG_TAB()]) {
    try { out.push(await pruneLogTab(tab, keepDays)); }
    catch (e) { out.push({ tab, error: e.message }); }
  }
  return out;
}

// Rebuild the "Low Indexation Sites" worklist tab from History: every auditable
// site with >= minPosts Seq+VB posts, ranked worst-first, with its current Status
// and a suggested action for sites below `threshold`. Overwrites the tab each run.
const LOWINDEX_TAB = () => process.env.GOOGLE_LOWINDEX_TAB || 'Low Indexation Sites';
export async function refreshLowIndexationTab({ threshold = 0.65, minPosts = 4 } = {}) {
  const numv = v => Number(String(v || '').replace(/[, ]/g, '')) || 0;
  const pmd = s => { const p = String(s).split('/').map(Number); return Date.UTC(p[2], p[0] - 1, p[1]); };
  const enc = t => encodeURIComponent(qtab(t));

  const g = await getTrackingSites({ limit: 0 });
  const allow = new Set(g.auditableDomains);

  // Current Status per domain — find the Tracker's domain + status columns, read computed values.
  const head = (await api(`/values/${enc(g.tab)}!A1:BZ12?valueRenderOption=FORMULA`)).values || [];
  const hr = head.findIndex(r => r.map(normHeader).some(h => h.includes('total indexed')));
  const hrow = head[hr] ? head[hr].map(normHeader) : [];
  let dCol = hrow.findIndex(h => h === 'domain' || h === 'url');
  if (dCol < 0 && hr > 0) dCol = head[hr - 1].map(normHeader).findIndex(h => h === 'domain' || h === 'url' || h.startsWith('domain'));
  const sCol = hrow.findIndex(h => h === 'status');
  const domVals = (await api(`/values/${enc(g.tab)}!${colLetter(dCol)}1:${colLetter(dCol)}?majorDimension=COLUMNS`)).values?.[0] || [];
  const stVals = sCol >= 0 ? ((await api(`/values/${enc(g.tab)}!${colLetter(sCol)}1:${colLetter(sCol)}?majorDimension=COLUMNS`)).values?.[0] || []) : [];
  const statusOf = {};
  for (let i = 0; i < domVals.length; i++) { const d = normDomain(domVals[i]); if (d) statusOf[d] = String(stVals[i] ?? '').trim(); }

  // Latest OK snapshot per site + persistence signal (weeks below threshold).
  const hist = await readTab(HISTORY_TAB());
  const by = {};
  for (const r of hist) {
    if (String(r['Status'] || '').toUpperCase() !== 'OK') continue;
    const dom = normDomain(r['Domain']); if (!allow.has(dom)) continue;
    const rd = pmd(r['Date']); if (Number.isNaN(rd)) continue;
    (by[dom] = by[dom] || []).push({ rd, seqT: numv(r['Seq Total']), seqI: numv(r['Seq Indexed']), vbT: numv(r['VB Total']), vbI: numv(r['VB Indexed']), postT: numv(r['Total Post']), postI: Math.min(numv(r['Total Indexed']), numv(r['Total Post'])) });
  }
  const out = [];
  for (const [dom, snaps] of Object.entries(by)) {
    snaps.sort((a, b) => a.rd - b.rd); const l = snaps[snaps.length - 1];
    const cT = l.seqT + l.vbT, cI = l.seqI + l.vbI; if (cT < minPosts) continue;
    const rate = cI / cT; const site = l.postT ? l.postI / l.postT : null;
    let wk = 0, low = 0;
    for (const s of snaps) { const st = s.seqT + s.vbT; if (st >= 1) { wk++; if ((s.seqI + s.vbI) / st < threshold) low++; } }
    let action = '';
    if (rate < threshold) action = (site != null && site < 0.15) ? 'QUARANTINE (site deindexed)' : (site != null && site >= 0.5) ? 'OPTIMIZE (posts not indexed)' : 'REVIEW';
    out.push([dom, statusOf[dom] || '(unknown)', action, +(rate * 100).toFixed(1), l.seqI, l.seqT, l.vbI, l.vbT, site != null ? +(site * 100).toFixed(1) : '', wk, low]);
  }
  out.sort((a, b) => a[3] - b[3] || (b[5] + b[7]) - (a[5] + a[7]));

  const headers = ['Domain', 'Current Status', 'Suggested Action', 'Combined %', 'Seq Indexed', 'Seq Total', 'VB Indexed', 'VB Total', 'Site %', 'Weeks Audited', `Weeks <${Math.round(threshold * 100)}%`];
  await ensureTab(LOWINDEX_TAB(), headers);
  await api(`/values/${enc(LOWINDEX_TAB())}!A1:Z100000:clear`, 'POST', {});
  await api(`/values/${enc(LOWINDEX_TAB())}!A1?valueInputOption=USER_ENTERED`, 'PUT', { values: [headers, ...out] });
  return { tab: LOWINDEX_TAB(), sites: out.length, flagged: out.filter(r => r[2]).length };
}
