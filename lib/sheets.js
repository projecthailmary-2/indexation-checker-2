...
export async function getTrackingSites({ limit = 0, failedOnly = false, freshDays = 0 } = {}) {
  ...
  for (let i = headerRow + 1; i < rows.length; i++) {
    ...
    sites.push({ domain: d, normDomain: normDomain(d), lastChecked: rawDate, ts: Number.isNaN(ts) ? -Infinity : ts, flagged, status: flagged ? indexedStr : '', auditable });
  }
  ...
  const auditableDomains = sites.filter(s => s.auditable).map(s => s.normDomain);
  ...
  return {
    ...
    total, auditableTotal, auditableDomains, neverChecked, flaggedCount, eligible, freshDays, failedOnly,
