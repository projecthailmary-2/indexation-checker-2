...
export async function getTrackingSites({ limit = 0, failedOnly = false } = {}) {
  ...
  const pool = failedOnly ? sites.filter(s => s.flagged) : sites;
  const selected = (limit && limit > 0) ? pool.slice(0, limit) : pool;

  return {
    tab, columns: colReport(cols),
    total, neverChecked, flaggedCount, failedOnly,
